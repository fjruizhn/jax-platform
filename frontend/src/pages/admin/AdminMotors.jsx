  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-sm font-semibold text-texto">{t.adminMotorsTitle}</h2>
        <button
          onClick={openCreate}
          className="text-xs px-3 py-1.5 rounded-lg bg-acento hover:bg-acento-hover text-sobre-color font-semibold transition-colors"
        >
          {t.adminMotorsCreate}
        </button>
      </div>

      <p className="text-xs text-texto-tenue mb-4">{t.adminMotorsLimitationNote}</p>
      ...
      <div className="rounded-lg border border-borde overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-borde/50">
            {motors.map(m => (
              <tr key={m.key} className="bg-hundido hover:bg-superficie transition-colors">
                <td className="px-4 py-3 font-medium text-texto font-mono text-xs">{m.key}</td>
                <td className="px-4 py-3 text-xs text-texto">{m.provider_id}</td>
                <td className="px-4 py-3 font-mono text-xs text-texto">{m.model_id}</td>
                <td className="px-4 py-3 text-xs text-texto-suave font-mono">{m.transport}</td>
                <td className="px-4 py-3 text-xs">
                  {m.dispatchable
                    ? <span className="text-exito">{t.adminMotorsDispatchableYes}</span>
                    : <span className="text-aviso">{t.adminMotorsDispatchableNo}</span>}
                </td>
                <td className="px-4 py-3 text-xs text-texto-suave">
                  {m.capabilities.length > 0
                    ? m.capabilities.map(c => c.capability_key).join(', ')
                    : <span className="text-texto-tenue">{t.adminMotorsNoCapabilities}</span>}
                </td>
                <td className="px-4 py-3 text-xs text-texto-suave">{m.status}</td>
              </tr>
            ))}
            {motors.length === 0 && (
              <tr><td colSpan={7} className="px-4 py-6 text-center text-xs text-texto-tenue">{t.adminMotorsEmpty}</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      {creating && (
        <div className="fixed inset-0 bg-fondo/70 flex items-center justify-center z-50">
          <div className="bg-superficie border border-borde rounded-xl p-6 w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <label className="block text-xs text-texto-suave mb-1">{t.adminModelsModelId}</label>
            <select
              value={form.model_id}
            ...
            <label className="block text-xs text-texto-suave mb-1">{t.adminBindingsTransport}</label>
            <select
            ...
            <p className="text-[10px] text-texto-tenue mb-3">{t.adminMotorsLimitationNote}</p>

            <div className="grid grid-cols-2 gap-3 mb-3">
              <div>
                <label className="block text-xs text-texto-suave mb-1">{t.adminMotorsMaxTokens}</label>
                <input
                  type="number"
                  min="0"
              ...
              <div>
                <label className="block text-xs text-texto-suave mb-1">{t.adminMotorsTimeout}</label>
                <input
                  type="number"
                  min="1"
            ...
            <label className="flex items-center gap-2 text-xs text-texto-suave mb-1">
              <input
                type="checkbox"
            ...
            <label className="flex items-center gap-2 text-xs text-texto-suave mb-4">
              <input
                type="checkbox"
            ...
            <label className="block text-xs text-texto-suave mb-2">{t.adminMotorsCapabilities}</label>
            <div className="max-h-40 overflow-y-auto border border-borde rounded-lg p-2 mb-4">
              {capabilities.map(cap => (
                <div key={cap.key} className="flex items-center gap-2 py-1">
                  ...
                  />
                  <span className="text-xs text-texto font-mono flex-1">{cap.key}</span>
            ...
            {saveError && <p className="text-xs text-peligro mb-3">{t.adminMotorsSaveError(saveError)}</p>}

            <div className="flex gap-2 justify-end">
              <button onClick={() => setCreating(false)} className="px-3 py-1.5 rounded-lg text-sm text-texto-suave hover:text-texto transition-colors">
                {t.adminCreateCancel}
              </button>
              <button
